...
add_to_log($course->id, 'zoom', 'view all', 'index.php?id=' . $course->id, '');
...
$PAGE->set_title(format_string("$course->shortname: $strname", true, array('context' => $context)));
...
foreach ($zooms as $z) {
    ...
    if ($finished) {
        ...
        if ($inprogress) {
            $label = html_writer::tag('span', $strmeetingstarted, array('class' => 'label label-info'));
